...
class BrokerAdapter(Protocol):
    ...
    def place_order(
        ...
        *,
        ...
        ``fill_price`` is always supplied by ``OrderManager``. Simulation
        adapters (``PaperBroker``) use it to produce a deterministic fill;
        real broker adapters (``KiteBrokerAdapter``) accept and ignore it,
        since a real broker determines its own execution price.
        """
        ...
