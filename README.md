...
## Technical-only multi-instrument portfolio backtester (`src/agentic_investing/portfolio/technical_only.py`, `scripts/run_technical_portfolio_backtest.py`)

A separate, multi-instrument (up to 200-name universe) daily-bar backtester used for multi-bagger-detection research, distinct from the single-instrument `Backtester` above. Best prior result over the 2021-09-05 to 2026-09-05 window was only **60.51%** total return (vs. NIFTYBEES buy-and-hold 45.71%) despite a sound trade-selection process (35.20% win rate, 2.37 profit factor, 1.62 Sharpe) — well short of the desired 200%+ multi-bagger-scale outcome. Root-caused via controlled experiments (not a signal-quality problem) to two compounding structural causes, both now fixed as opt-in (default-off) config:

- **Capital-deployment bug**: `RiskEngine.size_new_position` sizes every position as `risk_budget / stop_distance`. With the CLI's tiny default `risk_per_trade_fraction` (0.25%) and a wide ATR-based stop typical of the volatile small/mid-caps that actually become multi-baggers, every position was capped at only ~2.5-4% of account equity regardless of the 15%-of-equity `max_single_position_fraction` cap that should otherwise apply — average capital deployed across the whole 5-year run was only 24.5%. Added `RiskEngine.size_new_position_by_weight()` (capped by the same per-position/deployment/cash limits, but without dividing by the stop distance) and a new opt-in `OrderManager.submit_buy(size_by_weight=...)` parameter, defaulting to `False` so every existing live/shadow/agent caller is completely unaffected. `TechnicalOnlyConfig.use_weight_based_sizing` / `--use-weight-based-sizing` opts the research backtester into it.
- **Permanent kill-switch bug, which only appeared once position sizes were increased to fix the above**: the hard-drawdown kill switch (12% by default) never auto-resets, by design, for live/shadow trading (a human must call `reset_kill_switch()`). But the backtester dumps candidates into every available position slot starting on literally the first day of the backtest window with no ramp-up/staggering; a cluster of quick stop-outs in the first ~3 weeks of the 2021-09-05 window pushed mark-to-market drawdown past 12% and permanently froze the strategy at zero positions for the remaining ~4.7 years of every larger-position-size run tested. Added two more opt-in, default-off, research-only knobs, both leaving live/shadow/agent order flow completely untouched: `max_new_entries_per_period`/`new_entry_period_days` (caps new BUYs within a rolling window, so one bad week can't hit every open slot at once) and `backtest_kill_switch_cooldown_days` (auto-calls the same `reset_kill_switch()` a human would, but only after N calendar days, and only inside this one backtester's loop — extracted into a directly-unit-tested `_apply_kill_switch_cooldown()` helper).
- **Verified result** with all three enabled together (`--enable-pyramiding --use-weight-based-sizing --max-new-entries-per-period 2 --new-entry-period-days 5 --backtest-kill-switch-cooldown-days 30`, otherwise identical config): total return **160.18%** (CAGR 21.07%), average capital deployed 68.98%, Sharpe 1.40, max drawdown 18.66% (up from 8.81%, an accepted and disclosed trade-off for the much larger position sizes), kill switch tripped and auto-recovered 6 times. This is a large, honest improvement (60.51% → 160.18%) from fixing the actual structural bottleneck rather than re-tuning entry/exit signal parameters, though it still falls short of 200% — closing the remaining gap would need either more aggressive risk-taking (worse drawdown) or genuinely better entry/exit signal quality, not just sizing/pacing fixes.
- The stale `multibagger_analysis.md`'s claim that a 50-70 RSI entry gate excluded most real multi-baggers (median RSI 77.72 at first 2x crossing) was double-checked and found to be **already fixed** before this session — the CLI's `--maximum-rsi` default is already 100.

## Controlled agentic reasoning layer (Phase 7 — foundation)

Project-wide engineering rules are recorded in `AGENTS.md`. Prompt and instruction files live under `docs/prompts/` and are loaded at runtime; the application does not hardcode long-form prompts in Python source. Runtime logs are written under `logs/agentic_investing.log` (ignored by Git) with key tool, risk, order, and failure events. Never commit automatically — commits are only made when explicitly requested.

See [docs/zerodha-capability-matrix.md](docs/zerodha-capability-matrix.md) for the current Kite/MCP integration boundary, and [docs/ai-cost-estimate.md](docs/ai-cost-estimate.md) for provider costs and algorithm-first operating guidance.
...
- `agentic_investing.mcp_server`: an MCP (Model Context Protocol) tool server exposing the dynamically registered `AgentToolkit` tools to any MCP client. Current tools include `get_recent_bars`, `get_journal_history`, `get_daily_plan`, and `submit_trade_proposal`; current news/fundamentals research uses Claude's native `web_search` server tool instead of an unreliable third-party aggregator. Run with `python -m agentic_investing.mcp_server` (stdio transport). `submit_trade_proposal` remains the **only** tool that can ever touch a broker position; there is deliberately no `place_order` tool.
- `agentic_investing.agent.tools.AgentToolkit`: the shared, plain-function implementation behind the current MCP tools — used by *both* the generic MCP server and the scheduled runner, so there is exactly one implementation of `submit_trade_proposal`. **Tool schemas are never hand-maintained**: `TOOL_METHOD_NAMES` is the single list of tool method names, and both consumers derive JSON Schema directly from each method's type-annotated signature.
- `agentic_investing.agent.runner.AgentRunner`: provider-neutral tool-use loop. `create_model_client()` supports Claude, OpenAI, DeepSeek, Gemini-compatible APIs, and local Ollama inference behind one `ModelClient` protocol. Claude can use native web search; the other providers are explicitly marked as having no native web search and must prefer HOLD when current research is unavailable. The same structured proposal and deterministic execution boundary is used for every provider.
- `scripts/run_daily_agent_review.py`: explicit `--mode ai|algo` switch. `--mode algo` runs the deterministic SMA/shadow path without AI cost; `--mode ai --provider claude|openai|deepseek|gemini|ollama` runs provider-selected model reasoning. Both are paper/shadow-only and share the same risk/execution boundary. See `docs/ai-cost-estimate.md` for budgeting and cost controls.
- **Claude's native `web_search` tool** (enabled by default via `AgentRunConfig.enable_web_search`): current Indian-stock news/fundamentals research uses Claude's web search directly, reading moneycontrol.com, company filings, NSE/BSE circulars, investor-relations pages, and Indian financial press. Unofficial NSE/Screener.in/Trendlyne scraping is intentionally not used. `web_search` is a pure **server tool** — Anthropic executes it and returns results embedded in the response — so it never touches `AgentToolkit`/`ProposalExecutor` and has no path to a broker call; usage is counted separately and capped per instrument-run via `max_web_searches`.
